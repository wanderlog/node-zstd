...
function testStream(method, bufferFile, resultFile, done, params) {
  ...
  fs.createReadStream(path.join(__dirname, '/fixtures/', bufferFile)).pipe(method(params)).pipe(writeStream)
...
describe('ZSTD Stream', function () {
  describe('compress', function () {
    ...
    it('should compress text data with dict=(new Buffer("hello zstd"))', function (done) {
      testStream(zstd.compressStream, 'data.txt', 'data.txt.zst.09.dict', done, {
        level: 9,
  ...
  describe('decompress', function () {
    ...
    it('should decompress text data with dict=(new Buffer("hello zstd"))', function (done) {
      testStream(zstd.decompressStream, 'data.txt.zst.09.dict', 'data.txt', done, {
        level: 9,
        dict: Buffer.from('hello zstd'),
      })
    })
